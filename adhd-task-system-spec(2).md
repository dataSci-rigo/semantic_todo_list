...
Whether interest/energy/value guesses happen at capture time (slightly slower capture) or in a nightly batch (cheaper, but tasks sit unclassified); whether shopping lists should ever auto-clear purchased items or keep history; where the Haiku confidence threshold for auto-accepting a normalization match should sit before demanding a confirm tap; and how far to take consumable tracking — groceries (eggs, milk) deplete in a way a caulk gun doesn't, and a strict quantity system is likely more maintenance than it's worth, so the working assumption is boolean on_hand with the recipe checklist doubling as the moment inventory gets refreshed. None of these block starting Phase 0.
